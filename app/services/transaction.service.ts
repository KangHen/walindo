...
export const TransactionService = {
  async getAll(params?: Record<string, any>): Promise<TransactionListResponse> {
    ...
    if (error.value)
      throw new Error(error.value.message || "Failed to fetch transactions");
    return data.value as TransactionListResponse;
  },

  async getById(id: number | string): Promise<TransactionResponse> {
    const { data, error } = await useFetch<TransactionResponse>(
      `${API_URL}/${id}`
    );
    if (error.value)
      throw new Error(error.value.message || "Transaction not found");
  ...
  async update(
    ...
    if (error.value)
      throw new Error(error.value.message || "Failed to update transaction");
    return data.value as TransactionResponse;
  },
  ...
  async delete(id: number): Promise<void> {
    ...
    if (error.value)
      throw new Error(error.value.message || "Failed to delete transaction");
  },
};
